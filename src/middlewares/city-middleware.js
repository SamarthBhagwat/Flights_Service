...
const validateCreateRequest = function(req, res, next){
    if(req.body.name){
        ...
        let explanation = [];
        explanation.push("City name not found in request body");
...
const validateUpdateRequest = function(req, res, next){
    ...
    // - You cannot update id of the city once set
    // - updatedAt column needs to be changes everytime a successful request is passed. 
    if(req.body.id){
        const appError = new AppError("Cannot update id of city", StatusCodes.BAD_REQUEST);
        ErrorResponse.error = appError;
        res.status(ErrorResponse.error.statusCode).send(ErrorResponse);
    }
    ...
    for(const key in request){
        if(key == 'name'){
            ...
            const appError = new AppError(`Cannot update ${key} of the city`, StatusCodes.BAD_REQUEST);
            ErrorResponse.error = appError;
            res.status(ErrorResponse.error.statusCode).send(ErrorResponse);
        }
    }
